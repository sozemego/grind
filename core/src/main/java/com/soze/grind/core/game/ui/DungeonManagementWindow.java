...
public class DungeonManagementWindow extends Window {
  ...
  private final Table rootTable = new Table();
	...
	public DungeonManagementWindow(String title, WindowStyle style, UIElementFactory uiElementFactory) {
    super(title, style);
  ...
  private void resize() {
	...
	private void createSelectedTable(Dungeon dungeon) {
		...
		getTitleLabel().setText(dungeon.getName());

		Label label = uiElementFactory.createTextLabel();
		label.setText(dungeon.getName());
