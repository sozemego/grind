...
public class ResourceStorageTable extends Table {
  ...
  public ResourceStorageTable(ResourceStorage storage, UIElementFactory uiElementFactory) {
  ...
  private Table createResourceTable(ResourceEnum resource, int amount) {
    ...
    Image image =
        this.uiElementFactory.createImage("resource/" + resource.getName().toLowerCase() + ".png");
